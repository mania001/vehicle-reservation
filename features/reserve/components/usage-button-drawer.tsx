...
import { Camera, Fuel, Gauge, ImagePlus, MessageSquare, X } from 'lucide-react'
import Image from 'next/image'
import { useRef, useState } from 'react'
import { submitStartDrive } from '../actions/submit-start-drive'
...
export function UsageBottomDrawer({ reservation }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      setLoading(true)
      const result = await submitStartDrive({
    ...
    <Drawer open={open} onOpenChange={setOpen}>
      ...
      <DrawerContent className="max-h-[85vh] px-4 pb-4">
        <div className="mx-auto w-full max-w-md flex flex-col h-full overflow-hidden">
          ...
          <div className="space-y-8 overflow-y-auto px-1 flex-1">
            {/* 1. 주행 거리 */}
            ...
            <div className="space-y-3">
              <Label className="flex items-center gap-2 text-base">
                <MessageSquare size={18} /> 기타 특이사항
              </Label>
              <Textarea
                placeholder="파손이나 오염 등 기록할 내용이 있나요?"
